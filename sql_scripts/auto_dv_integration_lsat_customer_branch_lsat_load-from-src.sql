...
cte_lsat_set_row_num as (
    ...
    from auto_dv_integration.lsat_customer_branch tgt
    where exists (
        select 1
...
cte_lsat_latest_records as (
    select *
    from cte_lsat_set_row_num
    where row_num = 1
),
cte_stg_lsat_dedup_hsh_dif as (
    select *
    from (
        select
            *,
        ...
        from (
            ...
            union
            select * from cte_lsat_latest_records
        )
        window w as (
            partition by dv_hkey_lnk_customer_branch
...
select
	...
	dv_ldt,
    dv_ccd
from cte_stg_lsat_dedup_hsh_dif
where row_num = -1